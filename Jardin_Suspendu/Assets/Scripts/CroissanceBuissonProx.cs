...
public class CroissanceBuissonProx : MonoBehaviour
{
    public Tree t;
    ...
    void Start()
    {
        ...
        croissTime = Time.time + 2*growthDelay;
        //growthSpeed = 0.2f;
    ...
    void TrunkLength()
    {
        xTrunk += growthSpeed;
        yTrunk += growthSpeed;
        xTrunk = Mathf.Clamp(xTrunk, 7.0f, 7.3f);
        yTrunk = Mathf.Clamp(yTrunk, 8.0f, 8.3f);
    ...
    void TrunkRadius()
    {
        ...
        radTrunk = Mathf.Clamp(radTrunk, 0.7f, 1.0f);
    ...
    void BranchLength()
    {
        if (xBranch <= 5.0f && yBranch <= 5.2)
        {
            ...
            xBranch = Mathf.Clamp(xBranch, 7.5f, 8.0f);
            yBranch = Mathf.Clamp(yBranch, 7.7f, 8.2f);
    ...
    void BranchRadius()
    {
        ...
        radBranch = Mathf.Clamp(radBranch, 0.2f, 0.5f);
